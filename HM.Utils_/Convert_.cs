...
namespace HM.Utils_
{
    ...
    public static partial class Convert_
    {
        ...
        /// <summary>
        /// 时间转化
        /// 避免DateTime2 与 DateTime 的问题
        /// </summary>
        /// <param name="ts"></param>
        /// <returns></returns>
        public static DateTime ToDateTime_(this TimeSpan ts)
        {
            return DateTime.MinValue.AddYears(1970).Add(ts);
        }
        ...
        public static string ToString_(this TimeSpan ts)
        {
            return (ts.Days > 0 ? (ts.Days.ToString() + "天") : "")
                ...
                + (ts.Minutes > 0 ? (ts.Minutes.ToString() + "分钟") : "0分钟")
                + (ts.Seconds > 0 ? (ts.Seconds.ToString() + "秒") : "0秒");
        }
        #endregion
    }
}